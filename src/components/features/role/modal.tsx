import { RoleForm } from "@/components/features/role/form";
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <h5 className="mb-1">Xóa chức vụ</h5>

          <p className="mb-3 text-muted">
            Bạn có chắc muốn xóa chức vụ <strong>{item?.name}</strong>?
          </p>
        </div>
      </BaseModal>
    );
    ...
    >
      <div className="">
        {type && (type === "add" || type === "edit" || type === "detail") && (
          <RoleForm mode={type} role={item} onSubmit={onSubmit} />
        )}
      </div>
    </BaseModal>
  );
